...
%    1) t-test with p=0.05 for selecting features
...
close all
 ...
 load('brain_PC_MCI.mat') %Brain network sets based on different net parameters.
root=cd; addpath(genpath([root '/DATA'])); addpath(genpath([root '/FUN']));

nROI=size(brainNetSet{1,1},1); %ROI size
nSubj=length(lab); %number of subject
[nL nZ]=size(brainNetSet); % # of candidate parameter Lambda1 and Lambda2%%
p_value=0.05 ;% p_value is choosen from 0.01,0.05 and 0.1
%p-value of t-test for feature selection
...
acc=mean(Test_res) %accuracy 
sen=mean(Test_res(lab==1)) %sensitivity
spe=mean(Test_res(lab==-1)) %specificity
fprintf('this is MCI,p=0.05')
